    ...
    return (
        <div className="max-w-6xl mx-auto p-4 md:p-8">
            <header className="mb-12 text-center">
                <h1 className="text-4xl font-bold text-gray-900 mb-4">Elevate Your Practice</h1>
                <p className="text-xl text-gray-600">Choose a plan that fits your professional growth and reach more patients.</p>
            </header>

            {mySub && (
                <div className="mb-12 bg-blue-50 border border-blue-200 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between">
                    <div>
                        <h2 className="text-lg font-semibold text-blue-900">Current Plan: {mySub.plan.name}</h2>
                        <p className="text-blue-700">Status: <span className="capitalize font-bold">{mySub.status}</span> • Renews on {new Date(mySub.current_period_end).toLocaleDateString()}</p>
                    </div>
                    <button className="mt-4 md:mt-0 text-blue-600 font-medium hover:underline">Manage Billing</button>
                </div>
            )}

            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {plans.map((plan) => (
                    <div key={plan.id} className={`relative flex flex-col p-8 bg-white border rounded-3xl shadow-sm transition-all hover:shadow-xl ${plan.badge_type === 'gold' ? 'border-yellow-400 ring-2 ring-yellow-400 ring-opacity-50 scale-105 z-10' : 'border-gray-200'}`}>
                        ...
                        <div className="mb-8">
                            ...
                            <div className="flex items-baseline text-gray-900">
                                <span className="text-4xl font-extrabold tracking-tight">₹{plan.price_monthly}</span>
